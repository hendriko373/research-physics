...
{\color{red}{Due to the impossibility to deal with a non-constant cosmological term in the usual context of both general relativity and teleparallel gravity, many different dark energy models have been considered in the literature. For example, there are quin\-tes\-sence, phantom field, and dark energy models in which gravity is non-minimally couple to a scalar field. There are also $f(R)$ and $f(T)$ theories, which are generalisations of general relativity and teleparallel gravity, respectively. As an alternative approach, we have developed a new model in which dark energy is encoded in the background spacetime kinematics. This was obtained by replacing the underlying ordinary Poincar\'e-ruled special relativity by a de Sitter-ruled special relativity. Such replacement is motivated by prospective problems to conciliate ordinary special relativity with the existence of an invariant length parameter at the Planck scale. While preserving the constancy of the speed of light, the de Sitter group allows also the existence of an invariant length parameter.

The main achievement of this construction is that the cosmological term $\Lambda$ is no longer restricted to be constant by the second Bianchi identity. This means that its value can evolve with the cosmological time. This additional freedom, not present neither in general relativity nor in teleparallel gravity, makes this model suitable to describe the universe evolution, which as is well-known requires different values of the cosmological term at different epochs. For example, a huge cosmological term could drive inflation at the primordial universe. Afterwards, the cosmological term should decay to a small value in order to allow the formation of the structures we see today. Then, to account for the late--time acceleration in the universe expansion rate, the value of the cosmological term should somehow increase \cite{CCC}. Of course, as a whole new paradigm for the study of cosmology, further research needs to be conducted. For example, the 
field equations~\eqref{eq:fieldEqs_dStg_expl} 
and~\eqref{eq:field_eq_lambda} must be solved for a homogeneous 
and isotropic universe in order to check whether the time evolution of the 
cosmological function fits with observations. Another important point to be addressed is how the kinematic contribution in the deviation 
